...
- **Correctness scope:**
  ...
  - **Diagnostic plots state their limits (Phase 19+):** a visual diagnostic that suggests a conclusion (Q–Q normality plot → normal/non-normal; residual plot → heteroscedastic/mis-specified) must not be presented as a formal test and must disclose that it carries no p-value — the visual analog of the no-naked-p-value rule. Uncertainty bands extend the error-bar-semantics rule above: every confidence/prediction band names *which* interval it is (CI = mean response vs PI = new observation) **and** its level (e.g. 95%); an unlabeled band is a correctness violation. Promoted from the Phase 19 diagnostics work because it generalizes to every future diagnostic.
