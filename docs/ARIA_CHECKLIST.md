...
- [ ] **4. Keyboard navigation** — All list-based UI (series list, dataset list, saved plots strip) supports keyboard navigation. Arrow keys or Tab moves between items. Enter/Space activates. The series-editor modal's form controls across all nine chart types are keyboard-reachable, including the encoding/style controls (color-by, size-by law/min-max/size-legend fields, marker-shape and line-dash selects, the show-markers toggle, and marker-color) added through v2.14.0.
...
- Statistical annotation `.sr-only` spans verified for all nine chart types (scatter, line, bar, parity, contour, histogram, box, violin, heatmap)
