...
### Pair Plots / SPLOM `(scoped — docs-only pre-impl review COMPLETE; not yet implemented; standalone pulled-forward MINOR ahead of Phase 19)`
...
**§6 (measure at exit).** `chart.js` is **already at 301** (over the ~300 trigger) and grows with `renderPairPlot` + the branch; `modal.js` (338) and `modal-chart-fields.js` (330) are already over and each grows (chart-type button, save fields, the new column-picker panel); `data.js` (297) crosses via the `validateSeriesColumns` pair branch. **Recommendation: land the pre-identified `chart.js` split (extract the grid + whole-plot dispatch into `render-grid.js`) WITH this work** rather than a "reviewed, not split" deferral. New `renderers/pair.js` → §6/§7 co-review of `shared.js` + `pair.js`.
...
Deliverables (dependency order; §12 written UX flow for the new column-picker panel precedes the branch):
- [ ] `renderers/pair.js`: `buildPairTrace` — categorical hue via `categoryGroupsFromValues` (one splom trace per group), blank labeled diagonal, all-N²-axes themed, marker edges off; returns `{traces, layout, error, warning}`. §6/§7 co-review with `shared.js` (Data Viz + DS)
- [ ] Dispatcher: `renderPairPlot(plot, pd, series)` early branch in `renderOnePlot`; `pair` in `RENDERERS` + loop `continue` guard; wholesale layout merge; `bindRelayoutPersistence`. Land the `render-grid.js` §6 split here (EL)
- [ ] Modal: numeric-only multi-select column checklist (select-all/clear, default first 8, "N non-numeric excluded" hint) + categorical hue picker; chart-type button; save reads `pairCols` + the hard-block validation (EL + UX)
- [ ] `validateSeriesColumns` pair branch + `renderPairPlot` drop-missing/<2-floor policy; pairwise-n disclosure (hover count + diverging-n note) (DS §20)
- [ ] `state.js` schema comment; `## Schema` CHANGELOG line; `CODE_WALKTHROUGH`/`REVIEW_GUIDE` updates (§17)
- [ ] Tests: SPLOM of K columns yields K² cells with the right dimensions; categorical hue → one trace per group sharing dimensions; >8 warns / >12 blocks with the cell count; dropped column drops+warns and <2 hard-errors; pair blocked from grids + co-resident series (save + render); session round-trip; in-app Playwright verification (QA + DS)

Exit criteria: a pair plot renders an N×N matrix of the selected numeric columns with a categorical hue (one trace per group), a blank labeled diagonal, and no r by default; the column cap warns/blocks with an honest cell-count message; it is blocked from subplot grids and from sharing a plot; pairwise-n divergence is disclosed; dropped columns degrade gracefully; sessions round-trip; the §7 carve-out and §6 `chart.js` split are recorded; all prior tests green.
