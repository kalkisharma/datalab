...
| **Stab C** *(symbol picker partially shipped — see note)* | Styling model & controls | three-tier hierarchy discoverability (S/M, design-led); per-plot marker controls + symbol picker *(per-SERIES picker shipped ahead)* + "use global" toggle (M/L); line marker-toggle / line-style / width (M) | **#6 ⇄ #7 hard couple** — per-plot markers add a tier; must ship with the hierarchy explainer |
