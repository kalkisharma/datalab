...
**Phase 5–16:** Data cleaning UI, summary statistics, correlation matrices, distribution fits/KDE, hypothesis tests with effect sizes, computed columns, subplot grids, legend/colorbar polish — shipped. **Phase 17:** interpolated contours — shipped (v2.10.0). **Phase 18:** Plotly 3.x — shipped (v2.12.0). Workspace ergonomics shipped (v2.11.0); encoding & style controls shipped (v2.14.0); a run of visualization-polish releases — parity best-fit + readout controls, contour shading/iso-lines, and a colormap/colorbar overhaul (per-plot & per-series colormaps, editable colorbars, and a colorblind-safe **colormap-honesty fix**) — shipped (v2.15.0–v2.21.0). The tool is now a full visualization + cleaning + statistics workbench; the remaining scoped extension is **statistical diagnostics** (Phase 19) — see the phase sections below. *(This line read "designed in but not built yet" until the v2.10.0 doc review corrected the drift; the "later phases" list re-drifted as those phases shipped and was corrected again at the post-v2.14.0 review.)*
...
datalab/
  src/
    ...
    js/
      ...
      colorscales.js    — resolveColorscale (name → Plotly colorscale; explicit arrays + allowlist) + effectiveColormap (series>plot>global), v2.18.1/v2.20.0
      renderers/
...
| **Stab C** *(symbol picker + line controls shipped in v2.14.0 — see note)* | Styling model & controls | three-tier hierarchy discoverability (S/M, design-led); per-plot marker controls + symbol picker *(per-SERIES picker shipped v2.14.0)* + "use global" toggle (M/L); ~~line marker-toggle / line-style / width~~ *(shipped v2.14.0)* (M) | **#6 ⇄ #7 hard couple** — per-plot markers add a tier; must ship with the hierarchy explainer |
...
**Stab A exit — shipped `v2.13.0`** (ahead of Phase 19). Delivered: parity single-dataset compare + line color-by wired to per-category lines (the two 🔴); legend in-bounds clamp; parity-stats drag persistence (the annotation-in-export gap, root-caused to the relayout hook); SVG-rasterizes-WebGL notice; and the date-format prompt split to `date-prompt.js` (§6 — `modal.js` 323 → 286). Suite **183 + 7 BENCH** green *(claim corrected post-tag: the suite was actually 183 pass / 2 fail — two parity tests went red on the new join-dataset option; see the post-v2.13.0 review below)* (warm 15 ms, cold 627 ms, filter 45 ms, heap → baseline). **§6 sweep:** `index.html` 418 / `datatools.js` 304 (untouched) / `chart.js` **300** tolerated — chart.js is at the trigger after the relayout-hook growth; the relayout-persistence hook is named as the next split seam (→ `decorations.js`). New §16 **control-effect check satisfied** (line Color-by now renders). Additive — state v2, no migration, MINOR. **Stab B and Stab C remain queued** after Phase 19.

**Stab C — partial pull-forward (the per-series symbol picker).** The *per-series* marker-shape picker shipped ahead of Stab C as a standalone feature (commit `37fdffe`): an optional `series.style.symbol` for scatter/parity/line, wired through `buildMarkerStyle` (scatter/parity inherit it; line threads it by hand), with a "Marker shape" dropdown in the modal Style section. It follows the existing **"blank = global default"** per-series override pattern (identical to marker color/size), so it adds **no new styling tier** and needed no hierarchy explainer — which is exactly why it was safe to pull forward. The `#markerSymbol` global lookup is already in `buildMarkerStyle` so a future global control drops in with zero renderer change. **Still in Stab C:** the *per-plot* marker controls (the new tier that triggers #6⇄#7), the explicit **"use global"** toggle, the three-tier hierarchy discoverability/explainer, and the line marker-toggle/style/width items. A single cosmetic shape per series is not a data encoding (no §20 concern); shape-*by-category* (with a shape legend) remains a possible future item.

**Post-v2.13.0 full-team review.** The team re-reviewed the shipped Stab A surfaces (scatter/line/parity + plot settings, upload→export) and both docs. **Headline (measure-don't-reason, again):** the Stab A exit note above claimed "183 + 7 BENCH green," but a live run was **183 pass / 2 FAIL** — `log-axes.spec.js` and `subplots.spec.js` selected `#mJoinDataset {index:0}` expecting the cross-dataset join, and Stab A had inserted a new default index-0 "This dataset (two columns)" option, so both fell into single-dataset mode and timed out. The static pass had called the suite green by reading counts instead of running it — the slip §4 keeps catching. Fixed (point both at the join dataset, `index:1`); after three added honesty tests the suite is **188 + 7 BENCH green**. **Code findings actioned** (additive, warn-don't-block, §20): line color-by folds missing color values into one labelled `(blank)` group and warns; line color-by gains a >50 high-cardinality guard (boxplot/heatmap precedent); parity warns on same-column self-comparison (NSE=1 is trivial by construction). **Doc drift closed:** `date-prompt.js` added to the file tree; v2.13.0 CHANGELOG `## Schema` block; Phase 19's stale version placeholder and its already-done date-prompt chore. **Docs reorganized into `docs/`** (README stays at repo root as the entry point); added `REVIEW_GUIDE.md` (plain-language reviewer orientation) and `CODE_WALKTHROUGH.md` (full file-by-file technical walkthrough), both owned and updated at every phase exit per §17/§4. **One finding DEFERRED — named, not a silent carry (Accessibility; the NVDA-deferral precedent):** the draggable legend and parity-stats box (Stab A) have no keyboard path, and `ARIA_CHECKLIST.md` predates the current control set (9 chart types, color-/size-by). It surfaces at the next phase scoping; trip-wire = any external a11y report makes it blocking.

**Encoding & Style Controls — shipped `v2.14.0`** (pulled-forward features, not a scoped phase; version-at-ship-time §3). A batch of maintainer-requested encoding/style controls, all **additive — state v2, no migration, MINOR**: **size-by controls** (sizing law area/diameter with an honesty warning, editable min/max px, size-legend label/count/hide, and an opt-in second legend — `legend2`, draggable + persisted), **per-series marker shape** (`style.symbol`, scatter/parity/line), and **line-series controls** (markers toggle, separate marker colour, line dash) — which also **fixed the global Marker-size slider doing nothing on line plots** (line markers had a hardcoded 4 px; now honour the slider + per-series size via `buildMarkerStyle`). These satisfy Stab C's *symbol picker* and *line marker-toggle / line-style / width* items (per-SERIES scope only — the **per-plot** marker tier, the explicit **"use global"** toggle, and the three-tier hierarchy explainer remain Stab C). Suite **205 + 7 BENCH** green; the second legend's real-drag persistence and the line controls were browser-verified. **§6 sweep (recorded decisions):** `chart.js` 311, `modal-chart-fields.js` 306, `modal.js` 305 all over the ~300 trigger — **tolerated** this release with named seams (chart.js `plotly_relayout` hook → `decorations.js`; modal save/field-assembly extraction at the next modal change); `index.html` 421 (shell) / `datatools.js` 304 (untouched) as before. New test files: `size-by.spec.js`, `marker-shape.spec.js`, `line-style.spec.js`. **Earlier in the same span:** the post-v2.13.0 review (above) and the docs reorg into `docs/` + the two reviewer guides. DEPENDENCIES reassessment: no changes (v2.13.0 entry recorded retroactively here — missed at that tag).

### Visualization & Encoding Polish — SHIPPED `v2.15.0`–`v2.21.0`
A run of maintainer-requested, pulled-forward feature batches (not scoped phases; version-at-ship-time §3), all **additive — state v2, no migration, MINOR** except the colormap **fix** (`v2.18.1`, PATCH). Each was driven through the real app in-browser (Playwright + screenshots) and shipped with its `## Schema` line; the suite was green at every tag (**239 + BENCH** by v2.21.0).

- **`v2.15.0` Parity fits & faithful exports** — optional linear best-fit line + R² on parity; shared colour/opacity for the ±5%/±10% bands; larger marker range (slider→40, per-series→60). **Two export fixes:** PNG/SVG now render off-screen at a fixed size from a clone of the live layout (minor gridlines + zoom were dropped by `Plotly.downloadImage`'s responsive clone), and interactive zoom/pan now persists into `plotConfig` so a re-render keeps it. Parity stats box anchored to its own subplot cell (axis-domain refs).
- **`v2.16.0` Best-fit line styling** — colour / width / style (solid·dash·dot·dash-dot) for the parity fit line.
- **`v2.17.0` Contour shading & colorbar title** — "Smooth shading" toggle (heatmap gradient vs discrete grid-faithful bands) and an editable contour colorbar title.
- **`v2.18.0` Colorbar controls** — manual colour range (`zmin/zmax` or marker `cmin/cmax`), reverse colormap, hide/custom title, contour level count — across contour / heatmap / scatter·parity numeric color-by. Heatmap keeps its aggregation-named title (§20).
- **`v2.18.1` Colormap fix — PATCH, a live §20 honesty defect.** Six of the twelve menu colormaps (Plasma/Inferno/Magma/Coolwarm/Turbo/Reds) silently rendered as a wrong fallback — Plotly doesn't recognise those names — so the label disagreed with the colours, *and the colourblind-safe maps were among the broken ones*. New `colorscales.js` `resolveColorscale()` maps every option to an explicit colorscale and **allowlists** unknown values to Viridis (also closing a session-import gap); the picker is grouped CB-safe → diverging → single-hue → rainbow (cautioned). DS flagged it `blocks-phase`; shipped as its own fast PATCH ahead of the feature batch it seams into.
- **`v2.19.0` Parity best-fit reporting** — R² moved from the legend into the stats box; "show equation in legend" toggle; configurable significant figures.
- **`v2.20.0` Colormap overrides & contour lines** — per-plot and per-series colormap overriding the global (**series → plot → global**, resolved at the dispatcher's eff-clone so the trace cache reflects it with no renderer-contract change); a **mixed-scale warning** (DS) when one plot mixes colormaps/ranges; contour iso-lines / iso-labels+font / axis-grid toggles; separate colorbar title/tick font sizes. Contour modal regrouped (Colorbar / Contour lines / Shading / Scattered data).
- **`v2.21.0` Parity readout controls** — per-plot **Notes** visibility toggle (gated *inside* `appendNotes` so `_noteOffset` stays correct — a hidden note must not let a dragged stats box overwrite its saved position); per-series **which-stats** picker (NSE/MAE/RMSE/R²; deselect-all hides the box rather than drawing an empty one); **N moved to the legend** (`name (n=…)`, per-series toggle default on), falling back to the stats box when the legend is hidden and always kept in the screen-reader summary.
...
**Pre-impl review (full team — EL, Data Viz, Data Scientist/UX).** Three parallel reviews ran against the real seams. The review **reversed two locked decisions** on evidence and confirmed the architecture. Findings + maintainer rulings:

- **Whole-plot type, dispatched specially (EL + Data Viz).** A Plotly `splom` trace is inherently multi-axis: it owns its own `layout.grid` + `xaxis1..N`/`yaxis1..N`. That is the *same* ownership `renderOnePlot` claims for subplot grids (`chart.js:83`), and the per-cell merge loop only remaps the literal `xaxis`/`yaxis` keys (`chart.js:158`) — numbered axes would pass through unthemed/unremapped. So `pair` gets a **dedicated `renderPairPlot(plot, pd, series)` branch taken before the grid/loop machinery** (right after the `plotDivFor` guard, `chart.js:56`), not an in-loop `result.wholePlot` flag (which would scatter `if (!wholePlot)` guards across ~150 lines). It still calls `buildSeriesResult` (trace cache + dataset-revision keying preserved) and returns `{traces, layout, error}`, but the layout merges **wholesale** and the renderer themes all N² axes itself (via `plotTheme()`, as `contour.js` does). `pair` added to `RENDERERS`; an explicit `if (s.chartType === 'pair') continue;` belt-and-suspenders in the series loop.
- **Hard-blocked from subplot grids and co-resident series.** A pair plot owns the whole panel. **Save-time (primary):** the modal blocks saving a `pair` series into a gridded plot or a plot already holding another series (and the converse), with `err.textContent` + `return false`; the grid-creation control refuses a grid on a plot holding a pair series. **Render-time (backstop, hand-edited sessions, §8 fail-closed):** ignore a stray grid (pair owns its own) and render only the splom if other series co-exist, with a warning via the existing `showPanelErrors` channel.
- **Diagonal — REVERSED a locked assumption (Data Viz + DS).** The lock assumed histograms on the diagonal. **Plotly's `splom` cannot do this** — `diagonal.visible` only toggles the degenerate x-vs-x scatter, and the diagonal cell's y-axis is the shared *data-range* axis, not a *count* axis; a histogram needs the count axis, whose cell domain isn't known at `buildTrace` time. Native histograms would require **abandoning the single-splom trace for a hand-built N² subplot grid** (the seaborn/pandas model) — a materially larger build. **Maintainer ruling: v1 ships a blank, variable-name-labeled diagonal** (`diagonal: { visible: false }`); histograms deferred; KDE rejected (undisclosed bandwidth, §20).
- **Color-by — REVERSED a locked assumption (Data Viz + DS).** The lock put numeric color first, categorical deferred. The canonical pairplot value *is* categorical grouping (seaborn `hue`), and it's the **easier** build: it reuses the existing `categoryGroupsFromValues` one-trace-per-group idiom (one `splom` trace per group, identical `dimensions`/`showupper`/`showlower`/`diagonal`, palette color + legend entry), whereas numeric needs new splom cases in the dispatcher's colorbar logic (`chart.js:234`). A numeric ramp across many small cells reads as decorative. **Maintainer ruling: categorical groups first**, reusing the palette + the >`PALETTE.length` "colors repeat" warning and `(blank)`-group handling; numeric ramp deferred.
- **Columns & cap (DS/UX).** Numeric-only picker (a SPLOM cell is a pairwise numeric scatter; non-numeric have no meaning there) — a **multi-select checklist** with select-all/clear (NOT `makeDD`, which is single-select), in a Columns section at the top of the modal, with a non-silent "*N non-numeric excluded*" hint. **Maintainer ruling: default the first 8 numeric columns checked; soft-warn >8, hard-block >12** (N² readability — 8 cols = 64 cells; lower than the original 10/15 lock). The warning states the **cell count** (boxplot >50-categories precedent, `shared.js:18`). Cap is also guarded against `cols × groups` (categorical hue multiplies trace count); a groups cap (~6–8, palette size) and **dropped marker edges by default** (`marker.line.width:0` keeps splom on the regl/WebGL fast path — splom *is* WebGL-backed in 3.6.0, so rows are cheap; the N² cost is axes/layout).
- **Honesty (DS §20, mandatory regardless of options).** **No correlation r by default** — the scatter is the honest primitive, and the existing correlation heatmap (`renderCorrelation`) is already the r surface; the pair plot **points users there** rather than growing a second, possibly-divergent r surface. **Pairwise-complete deletion is disclosed:** each off-diagonal cell drops rows non-finite in either column, so cells can differ in n (`pearsonMatrix` already documents this hazard, `stats.js:57`) — expose plotted n per cell (hover point-count) and a note when n diverges across cells.
- **Dropped-column-on-reload (DS §20).** Diverges from the uniform block policy: a SPLOM that loses 1 of 8 columns is still a valid, honest 7×7. **Drop-missing + warn**; hard-error only if **<2 numeric columns survive**. The block-vs-drop decision lives in `renderPairPlot`, not the `chart.js:108` gate (the early branch bypasses it).

**Schema (additive — state stays v2, no migration, MINOR §3):** new `chartType` value `pair` (whole-plot); new optional `series.pairCols` (`string[]` of numeric columns; absent/null = all numeric, resolved at render); categorical hue reuses the existing `series.colorCol`. `xCol`/`yCol` unused. v2.x sessions load unchanged. `pairCols` members are column-name data (not a fixed enum) → no §8 allowlist; they reach the DOM only through the escaped modal builders.

**§7 amendment (comment-only — EL + Data Viz, the contract's first whole-plot carve-out; Phase 10 explicitly kept renderers single-axis-pair, PLANNING ~L497).** Document in `shared.js` that `pair` is dispatched specially: `renderOnePlot` intercepts it before the series loop, the renderer owns the entire figure layout, the returned `layout` merges wholesale (no per-cell remap), and the series may not co-exist with other series or a subplot grid — a documented carve-out from the single-axis-pair rule, not a change to it.

**§6 (measure at exit).** Pre-impl estimate was to land the `chart.js` split with this work; **at exit the decision was to tolerate** (see the recorded decision below the deliverables) — the grid + whole-plot dispatch is interleaved with the series loop, not a clean function-move, so the `render-grid.js` extraction stays a named seam for a dedicated refactor. New `renderers/pair.js` got the §6/§7 co-review of `shared.js` + `pair.js`.

**Sequencing.** Coupling with Phase 19 is **textual, not architectural** — both add chart types so both touch the same additive seams (`RENDERERS`, chart-type button list, modal assembly, schema comment, §6/§7), but the cores are disjoint (Phase 19's qq/residual are normal single-axis-pair renderers + specfun numerics; pair is a whole-plot early-branch). Ships **first, standalone**, as a named batch; Phase 19 keeps its number and ships later unaffected (§3: name = identity, number = plan order, version = ship order).

Deliverables (all shipped at v2.24.0):
- [x] `renderers/pair.js`: `buildPairTrace` — categorical hue via `categoryGroupsFromValues` (one splom trace per group), blank edge-labeled diagonal, all-N axes themed, marker edges off; returns `{traces, layout, error, warning}`.
- [x] Dispatcher: `renderPairPlot(plot, pd, series)` early branch in `renderOnePlot`; `pair` in `RENDERERS` + loop `continue` guard; wholesale layout merge; `bindRelayoutPersistence`.
- [x] Modal: numeric-only multi-select column checklist (Select all/Clear, default first 8, live cell-count, "N non-numeric excluded" hint) + categorical hue picker; chart-type button; save reads `pairCols` + the hard-block validation (both directions).
- [x] `validateSeriesColumns` pair branch + `renderPairPlot` drop-missing/<2-floor policy; pairwise-complete-n disclosure note (DS §20).
- [x] `state.js` schema comment; `## Schema` CHANGELOG line; `CODE_WALKTHROUGH`/`REVIEW_GUIDE`/`README` updates (§17).
- [x] `tests/pair.spec.js` (7 tests): N-dim SPLOM with blank diagonal; categorical hue → one trace per group sharing dimensions; dropped column warns + <2 hard-errors; >12 cap+warn; pairwise-n disclosure; co-resident hard-block; modal default-first-8 + cell-count. Full suite **252 + 7 skipped** green; in-app Playwright verification confirmed the 4×4 themed axis grid, per-group traces, and the disclosure/blocks.

**§6 (recorded decision).** `chart.js` (354), `modal.js` (366), `modal-chart-fields.js` (358) over the ~300 trigger — **tolerated** this release (house precedent, v2.14.0 tolerated the same three). The grid + whole-plot dispatch in `chart.js` is interleaved with the series loop (not a clean function-move), so its extraction (→ `render-grid.js`) stays the named seam for a dedicated refactor rather than riding a feature release.

**Verification note.** Splom is always WebGL-backed; headless Chromium's SwiftShader is rejected by Plotly's perf-caveat check, so the GL *rasterization* is unobservable in tests (the trace objects + the N×N themed `_fullLayout` grid are built regardless and were asserted). Real browsers with a GPU render it normally — the same family as the app's existing "SVG rasterizes WebGL" export notice.

**Follow-up — SVG rebuild, SHIPPED `v2.25.0` (defect fix).** The "real browsers with a GPU render it normally" assumption above was WRONG in the field: a user hit *"WebGL is not supported"* on real Edge **and** Firefox (hardware acceleration off / GPU blocklisted / VM — Plotly's `splom` requests a context with `failIfMajorPerformanceCaveat`, which refuses software rendering). `splom` is the only always-WebGL chart in DataLab; every other chart is SVG. A full-team review (DS §20: a blocks-phase-class defect — the chart rendered *nothing* and dead-ended the user to get.webgl.org) + a **proven prototype** (an SVG N×N grid rendered under WebGL-disabled Chromium where the splom failed) led to the fix: `pair.js` rebuilt on plain SVG `scatter` (off-diagonal) + `histogram` (diagonal) traces on a `layout.grid` — no WebGL, works everywhere, fixes the degraded SVG export, and unlocks the **histogram diagonal** (FD bins, matching the histogram chart type) that the splom's shared data-range axis could never draw. `renderPairPlot`, the §7 carve-out, the whole-plot dispatch, and the caps are unchanged; the schema is unchanged (state stays v2). Lost: splom's built-in linked brushing (unused by DataLab's static model). Verified in-app under WebGL-disabled Chromium (the exact failure surface); suite 253 green. This retroactively resolves the earlier "no r / blank diagonal" diagonal deferral — the honest FD-binned histogram diagonal is now the default.
...
- **UI placement → new chart types** (`qq`/`residual` in the series modal), not a Data Tools view — they inherit filters/joins/sessions/subplot-grids/saved-plots/export for free.
- **Confidence level → hard-code 95%** (matches the shipped tests' α=0.05); `tQuantile` generalizes, so a 90/95/99 selector can be added later with no rework.
- **Q–Q group-by → single column, no hue in the first cut** (matches the histogram type); per-group Q–Q is a later add reusing the pair hue pattern.
- **Ship model → one MINOR**, all three deliverables together; commits sequenced to isolate the `scatter.js` band surgery last.

**Design (DS §20 rulings — locked, not open):**
- **Q–Q (`qq`, 11th type):** sample quantiles (Y, raw) vs theoretical **standardized-z** quantiles (X); **Blom** plotting positions `(i−3/8)/(n+¼)` (best approximation to E[order statistics]; documented to differ from R's n≤10 switch); reference line through the **quartiles** (robust — a μ±σ line bends to the departures the plot reveals). Sample quartiles use the existing `quantile()` (no second convention). Linear axes only (signed quantiles — log is meaningless; warn if a log toggle is on). Optional per-group hue deferred.
- **Residual (`residual`, 12th type):** **raw** residuals (observed − fitted) vs fitted, zero reference line; X/Y + fit degree reusing `linearFit`/`polyFit` (reuse `series.trendDegree`, no new field). Own panel (axes differ from the source scatter). Standardized/studentized deferred (needs leverage we don't compute; dividing by s implies an outlier threshold we aren't testing). DS owns the guidance comment (funnel = heteroscedasticity → SEs/bands understated; curve = wrong model order). "Diagnoses, does not test."
- **CI/PI bands (`series.trendBands`: `none`|`ci`|`pi`|`both`, absent = none, §8 allowlist fail-closed):** **linear (degree-1) fit only** — `trendBands` + (degree>1 OR per-group) → **draw no bands + warn** (per-group-stays-linear precedent). `band = ŷ ± tQuantile(0.975, n−2)·s·√(k + 1/n + (x−x̄)²/Sxx)`, **k=0 for CI (mean response), k=1 for PI (new obs)** → CI ⊂ PI everywhere, minimized at x̄, flaring at the extremes; `n ≥ 3`. **Extend `linearFit` to also return `{sxx, meanX, ssRes}`** (additive, zero extra passes) rather than recompute — avoids a second missing-value path (§20 correctness). Filled `toself` traces UNDER the fit line (push before `fitLine`, PI before CI); legend **names them with the level** ("95% CI (mean response)" / "95% PI (new obs)" — unlabeled = §20 violation).
- **Numerics (`specfun.js`):** `normalInv` (probit) via **Acklam** rational approx (|err|<1.15e-9, optional Halley refinement using the existing `normalCdf`), guards p∉(0,1); `tQuantile(c, df)` by **bisection on the existing two-tailed `pTwoTailedT`** — target for the 97.5% point is `pTwoTailedT(t*,df)=2(1−c)=0.05` (the two-tailed trap), unique root (monotone), df→∞ → probit(0.975). References hand-derived/published: Φ⁻¹(0.975)=1.9599639845, Φ⁻¹(0.75)=0.6744897502, t(0.975,10)=2.228, t(0.975,1)=12.706; plus the `pTwoTailedT(tQuantile)=α` inverse round-trip and the tQuantile(∞)=probit cross-check.

**Integration notes (Data Viz + EL):**
- **No §7 change** — qq/residual are normal single-axis-pair renderers (compose in the loop, unlike pair's whole-plot carve-out).
- **Grid axis-title clobber (real gotcha):** in a subplot grid, `chart.js` overwrites cell axis titles with `s.xCol`/`s.yCol` and `autoXLabel`/`autoYLabel` push the column name into the left-panel inputs — clobbering "Theoretical/Sample"/"Fitted/Residuals". Add a small `diagnosticAxisLabel(series,'x'|'y')` so titles are right in grids and the inputs stay in sync.
- **CI/PI bands land in the degree-1 branch only** of `scatter.js` (~L219-228); the degree-2/3 and per-group paths stay untouched.
- **Schema (additive, state stays v2, MINOR):** new `chartType` `qq`/`residual`; new optional `series.trendBands`; residual reuses `series.trendDegree`, qq reuses `series.xCol` (+ future `colorCol`). `validateSeriesColumns` needs no new branch (both fall into the existing else). v2.0–v2.24 sessions load unchanged.
- **§6:** `specfun.js` 126 → ~165 (under trigger). `modal-chart-fields.js` (~358 → ~405 with the qq/residual field blocks + `trendBands` select) is the **split candidate for this phase**; `scatter.js` (~266 → ~300) records a review decision at exit.
...
### Post-v2.26.0 full-team docs + code audit
A maintainer-commissioned full-team audit (doc-vs-code accuracy, cross-doc consistency, DS §20 honesty, Security §8/§9, a11y, EL §6/§7/§3/§19 + code quality) of the v2.24.0–v2.26.0 work. Docs reached consensus after two rounds (recorded above / in the git history). The **code audit** confirmed the shipped statistics are sound (references independently re-derived, non-tautological), and §3/§7/§19 clean; it fixed the genuine findings:
- **§8 (Security, RISK):** the `RENDERERS[chartType]` dispatch used a truthiness guard, so a crafted session `chartType` that is an inherited `Object` member (`constructor`, `__proto__`, `toString`) passed and dead-ended in an uncaught TypeError (render DoS — no exec/pollution/exfil). Now fails closed via `Object.hasOwn` at both dispatch sites (`chart.js`, `render-cache.js`). `pair.js`'s `colData` map switched to `Object.create(null)` (a `__proto__`-named column can't hit the prototype setter).
- **§11 (robustness):** `Math.min(...arr)`/`Math.max(...arr)` spreads over full row arrays (new in `residual.js`/`scatter.js` bands + pre-existing fit paths) can `RangeError` at the 50k-row target — replaced with a single-pass `extent()` helper in `shared.js` (the idiom `areaSizes` already used). Also removed a redundant double `linearFit` in the band path.
- **§8 (evidence) + coverage:** added XSS-suite cases for the qq/residual column-name sinks; added the previously-missing tests for the diagnostic-axis-title **grid-clobber fix**, the `trendBands` **fail-closed allowlist**, band **z-order**, Q–Q **constant-column error** + log-axis warning, and residual **NaN-pair dropping**; removed dead code in a test (§19). Fixed a misleading escaping comment (the SR sink uses `textContent`, not escHtml).
- **§6 sweep (measure, don't recall — the recurring slip §4 targets):** re-measured every file. Over the ~300 trigger and **TOLERATED** with named seams: `modal.js` 369, `chart.js` 356 (grid + whole-plot dispatch → `render-grid.js`), `grid.js` 346 (→ `grid-cells.js`), `wiring.js` 343 (dropzone + modal wiring extractable — last decided v2.8.0, drift now recorded), `parity.js` 327 (stats/fit/bridge helpers extractable), `decorations.js` 320, `scatter.js` 313, `datatools.js` 304, `data.js` 303. None is forced to split by current work; each has a recorded decision now, closing the audit's §6 gap. Suite **274 + 7 skipped** green after the fixes.
